...
async function dashboard() {
  ...
  async function machinesView() {
    ...
    function renderRows(rows) {
      ...
      rows.forEach((m) => {
        const tr = create("tr");
        tr.innerHTML = `<td class="px-4 py-2">${m.name}</td><td class="px-4 py-2">${m.weightKg}</td><td class="px-4 py-2 text-right"><button data-id="${m.id}" class="text-red-600">×</button></td>`;
  ...
  async function dailyView() {
    ...
    machines.forEach((m) => {
      ...
      div.innerHTML = `<span class="flex-1">${m.name}</span><input type="number" class="input w-32" value="${m.weightKg}" data-id="${m.id}">`;
    ...
    form.onsubmit = async (e) => {
      e.preventDefault();
      const details = {};
      qs("#daily-machines").querySelectorAll("input").forEach((inp) => {
