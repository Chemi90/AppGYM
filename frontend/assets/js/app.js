...
async function dashboard(){
  ...
  async function render(){
    ...
    if(!tpl){
      console.error("Template no encontrado para:",view);
      return;
    }
    container.innerHTML="";
  ...
  async function profileView(){
    ...
    /* rellenar */
    ["firstName","lastName","age","height","weight"].forEach(id=>{
      if(data[id] || data[`${id}Cm`] || data[`${id}Kg`])
        form[id].value = data[id] ?? data[`${id}Cm`] ?? data[`${id}Kg`];
    });
  ...
  function statsView(){
    ...
    form.onsubmit = async e =>{
      ...
      await fetch(`${API_BASE}/api/stats`,{ method:"POST", headers:{ Authorization:`Bearer ${localStorage.getItem(TOKEN_KEY)}` }, body:fd });
  ...
  /* descarga: añade ?token=… y abre en nueva pestaña */
